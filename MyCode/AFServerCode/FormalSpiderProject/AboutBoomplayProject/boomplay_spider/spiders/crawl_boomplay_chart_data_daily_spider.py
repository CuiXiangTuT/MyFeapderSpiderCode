# -*- coding: utf-8 -*-
"""
Created on 2023-10-09 10:54:31
---------
@summary:
---------
@author: QiuQiuRen
...
class CrawlBoomplayChartDataDailySpider(feapder.BatchSpider):
    ...
    def add_task(self):
        sql = "UPDATE `chart_boomplay_batch_task` SET {}=0".format(
            self._task_state)
    ...
    def parse(self, request, response):
        ...
        chart_data['chart_name'] = response.xpath(
            './/section[@class="text"]/h1/text()').extract_first().lower()
        # 5.榜单部分
        ...
        chart_data['crawl_chart_country'] = str(
            re.findall(pattern, country)[0]).lower()
        # 8.更新频次
        ...
        for per_track_info in li_list:
            ...
            # chart_data_track_info['note'] = '除非洲以外榜单'
            # """
            # # 2.歌手任务表：boomplay_artist_info_batch_task_item
            # # """
            # artist_info_task_item = BoomplayArtistInfoBatchTaskItem()
            # artist_info_task_item[
            #     'boomplay_artist_id'] = chart_data_track_info[
            #     'chart_artist_id']
            # artist_info_task_item[
            #     'boomplay_artist_name'] = chart_data_track_info[
            #     'chart_artist_name']
            # # artist_info_task_item['source'] = 'data from the chart'
            #
            # """
            ...
            # # track_info_task_item['source'] = 'data from the chart'
            # """
            # 5-歌手歌曲映射表：
            # """
            # boomplay_artist_track_batch_data_item = BoomplayArtistTrackBatchDataItem()
            # boomplay_artist_track_batch_data_item['boomplay_artist_id'] = per_track_info.xpath(
            #     './/a[@class="artistName"]/@href').extract_first().split(
            #     '/')[-1].split('?')[0]
            # boomplay_artist_track_batch_data_item['track_id'] = per_track_info.xpath(
            #     './/a[@class="songName"]/@href').extract_first().split(
            #     '/')[-1].split('?')[0]
            # # boomplay_artist_track_batch_data_item['source'] = 'data from the chart'
...
if __name__ == "__main__":
    spider = CrawlBoomplayChartDataDailySpider(
    ...
    parser = ArgumentParser(description="CrawlBoomplayChartDataDailySpider爬虫")
    ...
    # 通过命令行启动
    # python crawl_boomplay_chart_data_daily_spider.py --start_master  # 添加任务
    # python crawl_boomplay_chart_data_daily_spider.py --start_worker  # 启动爬虫
