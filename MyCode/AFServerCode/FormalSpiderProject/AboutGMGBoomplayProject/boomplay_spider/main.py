...
#         batch_record_table="record_af_crawl_boomplay_chart_data_daily_spider",  # mysql中的批次记录表
#         batch_name="1-Boomplay每日榜单数据(非洲)",  # 批次名字
...
#         redis_key="feapder_boomplay_af:crawl_gmg_boomplay_chart_data_daily_spider",  # redis中存放request等信息的根key
#         task_state="af_state",  # mysql中任务状态字段
#     )
#
#     if args == 1:
...
def about_crawl_boomplay_artist_info_spider(args):
    """
    2-Boomplay GMG歌手信息采集
    """
    spider = crawl_boomplay_artist_info_spider.CrawlBoomplayArtistInfoSpider(
        task_table="gmg_boomplay_artist_info_batch_task",  # mysql中的任务表
        batch_record_table="record_af_gmg_boomplay_artist_info_batch_task",  # mysql中的批次记录表
        batch_name="2-Boomplay GMG歌手信息采集(非洲)",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id", "gmg_artist_id", "gmg_artist_name", "boomplay_artist_id", "boomplay_artist_name"],  # 需要获取任务表里的字段名，可添加多个
        redis_key="feapder_boomplay_af:crawl_gmg_boomplay_artist_info_spider",  # redis中存放request等信息的根key
        task_state="af_state",  # mysql中任务状态字段
    )
...
def about_crawl_boomplay_artist_songs_albums_playlists_counts_spider(args):
    ...
    spider = crawl_boomplay_artist_songs_albums_playlists_counts_spider.CrawlBoomplayArtistSongsAlbumsPlaylistsCountsSpider(
        task_table="gmg_boomplay_artist_info_batch_task",  # mysql中的任务表
        batch_record_table="record_af_gmg_boomplay_artist_songs_albums_playlists_counts_batch",  # mysql中的批次记录表
        batch_name="3-Boomplay GMG歌手歌曲、专辑、播放列表 条目数 采集(非洲)",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id", "boomplay_artist_id","gmg_artist_id"],  # 需要获取任务表里的字段名，可添加多个
        redis_key="feapder_boomplay_af:crawl_gmg_boomplay_artist_songs_albums_playlists_counts_spider",  # redis中存放request等信息的根key
        task_state="af_crawl_songs_albums_playlists_count_state",  # mysql中任务状态字段
        task_condition="af_crawl_songs_albums_playlists_count_state=0"
    )
...
def about_crawl_boomplay_artist_songs_albums_playlists_task_spider(args):
    ...
    spider = crawl_boomplay_artist_songs_albums_playlists_task_spider.CrawlBoomplayArtistAlbumTrackInfoTaskSpider(
        task_table="gmg_boomplay_artist_info_batch_task",  # mysql中的任务表
        batch_record_table="record_af_gmg_boomplay_artist_songs_albums_playlists_task_batch",  # mysql中的批次记录表
        batch_name="4-Boomplay GMG歌手歌曲id、专辑id、播放列表id 及 相关映射 采集(非洲)",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id", "boomplay_artist_id"],  # 需要获取任务表里的字段名，可添加多个
        redis_key="feapder_boomplay_af:crawl_gmg_boomplay_artist_songs_albums_playlists_task_spider",  # redis中存放request等信息的根key
        task_state="af_crawl_artist_album_track_playlist_task_state",  # mysql中任务状态字段
        task_condition="af_crawl_artist_album_track_playlist_task_state=0"
    )
...
def about_crawl_boomplay_album_info_spider(args):
    ...
    spider = crawl_boomplay_album_info_spider.CrawlBoomplayAlbumInfoSpider(
        task_table="gmg_boomplay_album_info_batch_task",  # mysql中的任务表
        batch_record_table="record_af_gmg_boomplay_album_info_batch_task_batch",  # mysql中的批次记录表
        batch_name="5-Boomplay GMG专辑信息及专辑歌曲映射、专辑下歌曲任务采集(非洲)",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id", "album_id"],  # 需要获取任务表里的字段名，可添加多个
        redis_key="feapder_boomplay_af:crawl_gmg_boomplay_album_info_spider",  # redis中存放request等信息的根key
        task_state="af_state",  # mysql中任务状态字段
    )
...
def about_crawl_boomplay_track_info_spider(args):
    ...
    spider = crawl_boomplay_track_info_spider.CrawlBoomplayTrackInfoSpider(
        task_table="gmg_boomplay_track_info_batch_task",  # mysql中的任务表
        batch_record_table="record_af_gmg_boomplay_track_info_batch_task",  # mysql中的批次记录表
        batch_name="6-Boomplay GMG歌曲信息采集 (非洲) ",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id",'track_id'],  # 需要获取任务表里的字段名，可添加多个
        redis_key="feapder_boomplay_af:crawl_gmg_boomplay_track_info_spider",  # redis中存放request等信息的根key
...
def about_crawl_boomplay_track_views_spider(args):
    ...
    spider = crawl_boomplay_track_views_spider.CrawlBoomplayTrackViewsSpider(
        task_table="gmg_boomplay_track_info_batch_task",  # mysql中的任务表
        batch_record_table="record_af_gmg_boomplay_track_views_batch_task",  # mysql中的批次记录表
        batch_name="8-Boomplay GMG批次播放量采集(非洲)",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id","track_id"],  # 需要获取任务表里的字段名，可添加多个
        redis_key="feapder_boomplay_af:crawl_gmg_boomplay_track_views_spider",  # redis中存放request等信息的根key
...
if __name__ == "__main__":
    parser = ArgumentParser(description="GMG Boomplay相关爬虫(非洲)")
    ...
    parser.add_argument(
        ...
        type=int,
        nargs=1,
        help="3- GMGBoomplay歌手歌曲、专辑、播放列表 条目数 采集(非洲)",
        choices=[1, 2, 3],
    ...
    parser.add_argument(
        ...
        type=int,
        nargs=1,
        help="4-Boomplay GMG歌手歌曲id、专辑id、播放列表id 及 相关映射 采集(非洲)",
        choices=[1, 2, 3],
    ...
    parser.add_argument(
        ...
        type=int,
        nargs=1,
        help="5-Boomplay GMG专辑信息及专辑歌曲映射、专辑下歌曲任务采集(非洲)",
        choices=[1, 2, 3],
