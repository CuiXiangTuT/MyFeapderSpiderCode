    ...
    def parse(self, request, response):
        ...
        if songs_count:
            ...
            for per_li in li_list:
                ...
                # 【歌手-歌曲映射】
                boomplay_artist_track_batch_data_item = GmgBoomplayArtistTrackBatchDataItem()
        ...
        if albums_count:
            ...
            for per_album_info in album_info_list:
                # 【专辑任务表】
                boomplay_album_info_batch_task_item = GmgBoomplayAlbumInfoBatchTaskItem()
                boomplay_album_info_batch_task_item["album_id"] = \
                per_album_info.xpath(".//a/@href").extract_first().split('/')[-1].split('?')[0]

                # 【歌手 专辑映射】
                boomplay_artist_album_batch_data_item = GmgBoomplayArtistAlbumBatchDataItem()
        ...
        if playlists_count:
            ...
            for per_li in playlists_info_list:
                # 【播放列表任务表】
                boomplay_playlist_info_batch_task_item = GmgBoomplayPlaylistInfoBatchTaskItem()
                boomplay_playlist_info_batch_task_item['boomplay_playlist_id'] = per_li.xpath(".//a/@href").extract_first().split('/')[-1].split('?')[0]
                # 【歌手 播放列表 映射】
                boomplay_artist_playlist_batch_data_item = GmgBoomplayArtistPlaylistBatchDataItem()
