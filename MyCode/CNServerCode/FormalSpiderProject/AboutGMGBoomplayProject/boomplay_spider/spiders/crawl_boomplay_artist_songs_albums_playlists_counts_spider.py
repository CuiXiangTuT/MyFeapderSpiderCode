...
class CrawlBoomplayArtistSongsAlbumsPlaylistsCountsSpider(feapder.BatchSpider):
    ...
    def start_requests(self, task):
        ...
        yield feapder.Request(url, task_id=task_id,task_gmg_artist_id=task_gmg_artist_id, task_boomplay_artist_id=task_boomplay_artist_id)

    def parse(self, request, response):
        # 检测歌手存在Songs、Albums、Playlists标签
        boomplay_artist_songs_albums_playlists_counts_record_batch_data_item = GmgBoomplayArtistSongsAlbumsPlaylistsCountsRecordDataItem()
        is_exists_banner = response.xpath('//div[contains(@class,"tab_label") and contains(@class,"clearfix")]')
        if is_exists_banner:
            boomplay_artist_songs_albums_playlists_counts_record_batch_data_item['gmg_artist_id'] = request.task_gmg_artist_id
            ...
            if boomplay_artist_songs_albums_playlists_counts_record_batch_data_item['songs_count'] == \
                ...
                # -->bj_crawl_artist_album_track_state置为-3，在采集歌曲、专辑、播放列表信息时直接过滤，不作采集
