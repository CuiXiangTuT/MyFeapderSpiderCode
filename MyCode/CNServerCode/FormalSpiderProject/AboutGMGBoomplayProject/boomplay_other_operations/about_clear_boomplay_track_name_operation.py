...
def main():
    ...
    sql = 'SELECT track_id,track_name FROM `gmg_boomplay_track_name_cleaned_data` WHERE state=0'
    cursor.execute(sql)
    ...
    for tuple_track_info in data:
        ...
        print("-------------------------------------------------------------")
        sql_update = """UPDATE boomplay_data.b_gmg_boomplay_track_name_cleaned_data SET track_name_cleaned=%s,feat_artist_name=%s,version=%s,state=1 WHERE track_id=%s AND state = 0"""
        cursor.execute(sql_update,
                       [track_name_cleaned.lower(), feat_name.lower(), version.lower(), tuple_track_info[0]])
        conn.commit()

        sql_update_1 = """UPDATE gmg_boomplay_track_name_cleaned_data SET track_name_cleaned=%s,feat_artist_name=%s,version=%s,state=1 WHERE track_id=%s AND state = 0"""
