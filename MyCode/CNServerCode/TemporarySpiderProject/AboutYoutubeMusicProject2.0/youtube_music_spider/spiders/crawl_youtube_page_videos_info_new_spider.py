# -*- coding: utf-8 -*-
"""
Created on 2024-01-11 18:41:54
---------
@summary:
---------
@author: QiuQiuRen
...
class CrawlYoutubePageVideosInfoNewSpider(feapder.BatchSpider):
    def init_task(self):
        pass
        ...
        if request.nextContinuationData == None:
            ...
            if response.json.get("header"):
                ...
                if response.json["header"]["musicDetailHeaderRenderer"].get("secondSubtitle"):
                        secondSubtitle_list = response.json["header"]["musicDetailHeaderRenderer"]["secondSubtitle"][
                            "runs"]
                        if len(secondSubtitle_list) == 3:
                            # origin_songs_count
                            origin_songs_count = \
                                response.json["header"]["musicDetailHeaderRenderer"]["secondSubtitle"]["runs"][0][
                                    "text"]
                            ...
                            origin_total_duration = \
                                response.json["header"]["musicDetailHeaderRenderer"]["secondSubtitle"]["runs"][-1][
                                    "text"]
            ...
            json_data = response.json["contents"]["singleColumnBrowseResultsRenderer"]["tabs"][0]["tabRenderer"]["content"][
            ...
            for per_json_data in json_data:
                ...
                if per_json_data["musicResponsiveListItemRenderer"]["flexColumns"][0][
                        "musicResponsiveListItemFlexColumnRenderer"]["text"]["runs"][0]["navigationEndpoint"].get("watchEndpoint"):
                    ...
                    youtube_music_artist_plate_data_new_item[
                        "youtube_music_video_url_split_playlist_id"] = None
                    youtube_music_artist_plate_data_new_item[
                        "youtube_music_video_url_split_playlist_url"] = None
...
if __name__ == "__main__":
    spider = CrawlYoutubePageVideosInfoNewSpider(
    ...
    parser = ArgumentParser(description="CrawlYoutubePageVideosInfoNewSpider爬虫")
    ...
    # 通过命令行启动
    # python crawl_youtube_page_videos_info_new_spider.py --start_master  # 添加任务
    # python crawl_youtube_page_videos_info_new_spider.py --start_worker  # 启动爬虫
