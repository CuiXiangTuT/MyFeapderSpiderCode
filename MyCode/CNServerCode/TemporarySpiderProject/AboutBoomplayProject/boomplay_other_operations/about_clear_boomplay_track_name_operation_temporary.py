...
def main():
    MYSQL_IP = TEMPORARY_BJ_MYSQL_IP
    MYSQL_PORT = TEMPORARY_BJ_MYSQL_PORT
    MYSQL_DB = TEMPORARY_BJ_MYSQL_DB
    MYSQL_USER_NAME = TEMPORARY_BJ_MYSQL_USER_NAME
    MYSQL_USER_PASS = TEMPORARY_BJ_MYSQL_USER_PASS
    conn = pymysql.Connect(host=MYSQL_IP, user=MYSQL_USER_NAME, password=MYSQL_USER_PASS, port=MYSQL_PORT, db=MYSQL_DB)
    cursor = conn.cursor()

    sql = 'SELECT `曲库_NO.`,`Track Name` FROM `7w曲库75915_copy1` WHERE state=0  AND `Track Name` IS NOT NULL'
    cursor.execute(sql)
    ...
    for tuple_track_info in data:
        ...
        sql_update = """UPDATE `7w曲库75915_copy1` SET feat_cleaned=%s,track_version_cleaned=%s,state=1 WHERE `曲库_NO.`=%s AND state = 0"""
        cursor.execute(sql_update,
                       [feat_name.lower(), version.lower(), tuple_track_info[0]])
        conn.commit()
