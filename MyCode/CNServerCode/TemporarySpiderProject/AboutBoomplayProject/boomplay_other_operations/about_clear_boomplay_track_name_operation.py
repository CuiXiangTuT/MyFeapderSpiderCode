...
"""
@description：
    目的：对boomplay_track_info_batch_data中的歌曲名进行清理，将feat及版本信息提取出来，并更新入库
"""
...
def main():
    MYSQL_IP = BJ_MYSQL_IP
    MYSQL_PORT = BJ_MYSQL_PORT
    MYSQL_DB = BJ_MYSQL_DB
    MYSQL_USER_NAME = BJ_MYSQL_USER_NAME
    MYSQL_USER_PASS = BJ_MYSQL_USER_PASS
    conn = pymysql.Connect(host=MYSQL_IP, user=MYSQL_USER_NAME, password=MYSQL_USER_PASS, port=MYSQL_PORT, db=MYSQL_DB)
    cursor = conn.cursor()

    sql = 'SELECT track_id,track_name FROM `b_gmg_boomplay_track_name_cleaned_data` WHERE state=0'
    cursor.execute(sql)
    ...
    for tuple_track_info in data:
        ...
        print("-------------------------------------------------------------")
        sql_update = """UPDATE b_gmg_boomplay_track_name_cleaned_data SET track_name_cleaned=%s,feat_artist_name=%s,version=%s,state=1 WHERE track_id=%s AND state = 0"""
        cursor.execute(sql_update,
                       [track_name_cleaned.lower(), feat_name.lower(), version.lower(), tuple_track_info[0]])
        conn.commit()
