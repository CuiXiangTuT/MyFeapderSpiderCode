...
def about_crawl_boomplay_chart_data_daily_spider(args):
    ...
    spider = crawl_boomplay_chart_data_daily_spider.CrawlBoomplayChartDataDailySpider(
        ...
        batch_record_table="record_crawl_temporary_boomplay_chart_data_daily_spider",  # mysql中的批次记录表
        ...
        redis_key="feapder_temporary:crawl_temporary_boomplay_chart_data_daily_spider",  # redis中存放request等信息的根key
...
def about_crawl_boomplay_artist_info_spider(args):
    ...
    spider = crawl_boomplay_artist_info_spider.CrawlBoomplayArtistInfoSpider(
        ...
        batch_record_table="record_temporary_boomplay_artist_info_batch_task",  # mysql中的批次记录表
        batch_name="2-Boomplay GMG歌手信息采集",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id", "gmg_artist_id", "gmg_artist_name", "boomplay_artist_id", "boomplay_artist_name"],  # 需要获取任务表里的字段名，可添加多个
        redis_key="feapder_temporary:crawl_temporary_boomplay_artist_info_spider",  # redis中存放request等信息的根key
        task_state="state",  # mysql中任务状态字段
    )
...
def about_crawl_boomplay_artist_songs_albums_playlists_counts_spider(args):
    ...
    spider = crawl_boomplay_artist_songs_albums_playlists_counts_spider.CrawlBoomplayArtistSongsAlbumsPlaylistsCountsSpider(
        task_table="boomplay_artist_info_batch_task",  # mysql中的任务表
        batch_record_table="record_temporary_boomplay_artist_songs_albums_playlists_counts_batch",  # mysql中的批次记录表
        batch_name="3-Boomplay GMG歌手歌曲、专辑、播放列表 条目数 采集",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id", "boomplay_artist_id","gmg_artist_id"],  # 需要获取任务表里的字段名，可添加多个
        redis_key="feapder_temporary:crawl_temporary_boomplay_artist_songs_albums_playlists_counts_spider",  # redis中存放request等信息的根key
...
def about_crawl_boomplay_artist_songs_albums_playlists_task_spider(args):
    ...
    spider = crawl_boomplay_artist_songs_albums_playlists_task_spider.CrawlBoomplayArtistAlbumTrackInfoTaskSpider(
        task_table="boomplay_artist_info_batch_task",  # mysql中的任务表
        batch_record_table="record_temporary_boomplay_artist_songs_albums_playlists_task_batch",  # mysql中的批次记录表
        batch_name="4-Boomplay GMG歌手歌曲id、专辑id、播放列表id 及 相关映射 采集",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id", "boomplay_artist_id"],  # 需要获取任务表里的字段名，可添加多个
        redis_key="feapder_temporary:crawl_temporary_boomplay_artist_songs_albums_playlists_task_spider",  # redis中存放request等信息的根key
...
def about_crawl_boomplay_album_info_spider(args):
    ...
    spider = crawl_boomplay_album_info_spider.CrawlBoomplayAlbumInfoSpider(
        ...
        batch_record_table="record_temporary_boomplay_album_info_batch_task_batch",  # mysql中的批次记录表
        batch_name="5-Boomplay GMG专辑信息及专辑歌曲映射、专辑下歌曲任务采集",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id", "album_id"],  # 需要获取任务表里的字段名，可添加多个
        redis_key="feapder_temporary:crawl_temporary_boomplay_album_info_spider",  # redis中存放request等信息的根key
...
def about_crawl_boomplay_track_info_spider(args):
    ...
    spider = crawl_boomplay_track_info_spider.CrawlBoomplayTrackInfoSpider(
        ...
        batch_record_table="record_temporary_boomplay_track_info_batch_task",  # mysql中的批次记录表
        batch_name="6-Boomplay GMG歌曲信息采集  ",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id",'track_id'],  # 需要获取任务表里的字段名，可添加多个
        redis_key="feapder_temporary:crawl_temporary_boomplay_track_info_spider",  # redis中存放request等信息的根key
        task_state="state",  # mysql中任务状态字段
    )
    if args == 1:
        spider.start_monitor_task()
...
def about_crawl_boomplay_track_views_spider(args):
    ...
    spider = crawl_boomplay_track_views_spider.CrawlBoomplayTrackViewsSpider(
        task_table="boomplay_track_info_batch_task",  # mysql中的任务表
        batch_record_table="record_temporary_boomplay_track_views_batch_task",  # mysql中的批次记录表
        batch_name="8-Boomplay GMG批次播放量采集",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id","track_id"],  # 需要获取任务表里的字段名，可添加多个
        redis_key="feapder_temporary:crawl_temporary_boomplay_track_views_spider",  # redis中存放request等信息的根key
...
if __name__ == "__main__":
    parser = ArgumentParser(description="GMG Boomplay相关爬虫")
    ...
    parser.add_argument(
        ...
        type=int,
        nargs=1,
        help="3- GMGBoomplay歌手歌曲、专辑、播放列表 条目数 采集",
        choices=[1, 2, 3],
    ...
    parser.add_argument(
        ...
        type=int,
        nargs=1,
        help="4-Boomplay GMG歌手歌曲id、专辑id、播放列表id 及 相关映射 采集",
        choices=[1, 2, 3],
    ...
    parser.add_argument(
        ...
        type=int,
        nargs=1,
        help="5-Boomplay GMG专辑信息及专辑歌曲映射、专辑下歌曲任务采集",
        choices=[1, 2, 3],
