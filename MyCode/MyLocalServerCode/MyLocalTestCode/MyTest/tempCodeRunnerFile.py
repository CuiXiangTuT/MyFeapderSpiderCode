...
class MyTestAirspider(feapder.AirSpider):
    ...
    def start_requests(self):   
        ...
        # url="https://music.youtube.com/channel/UClyA28-01x4z60eWQ2kiNbA"
    ...
    def parse(self, request, response):
        ...
        if len(matches) >= 2:
            ...
            if match:
                ...
                # 7-歌手所有歌曲的URL
                if "navigationEndpoint" in json_data["contents"]["singleColumnBrowseResultsRenderer"]["tabs"][0]["tabRenderer"]["content"]["sectionListRenderer"]["contents"][0]["musicCarouselShelfRenderer"]["header"]["musicCarouselShelfBasicHeaderRenderer"]["title"]["runs"][0]:
                    ...
                    item["youtube_music_all_songs_url"] = None
                ...
                for k in musicCarouselShelfRenderer_data_list:
                    ...
                    if key_label=="专辑":
                        if "navigationEndpoint" in k["musicCarouselShelfRenderer"]["header"]["musicCarouselShelfBasicHeaderRenderer"]["title"]["runs"][0]:
                            ...
                            item["youtube_music_all_albums_singles_url"] = None
                            album_list = k["musicCarouselShelfRenderer"]["contents"]
                            for album_per in album_list:
                                album_item = dict()
                                album_item["album_id"] = album_per['musicTwoRowItemRenderer']['navigationEndpoint']['browseEndpoint']['browseId']
                                print("album_item:",album_item)
                    elif key_label=="单曲":
                        if "navigationEndpoint" in k["musicCarouselShelfRenderer"]["header"]["musicCarouselShelfBasicHeaderRenderer"]["title"]["runs"][0]:
                            item["youtube_music_all_albums_singles_url"] = k["musicCarouselShelfRenderer"]["header"]["musicCarouselShelfBasicHeaderRenderer"]["title"]["runs"][0]["navigationEndpoint"]["browseEndpoint"]["browseId"]
                        # else:
