...
def main():
    ...
    cursor = conn.cursor()
    # insert_sql = """
    # INSERT IGNORE INTO `temporary_boomplay_track_name_cleaned_data`(track_id,track_name)
    # SELECT boomplay_track_id,TRIM(LOWER(capture_track_name_boomplay)) track_name FROM `check_20230922_001_bak_20230925_nigeria`
    # """
    # cursor.execute(insert_sql)
    # conn.commit()

    sql='SELECT track_id,track_name FROM `temporary_boomplay_track_name_cleaned_data` WHERE state=0'
    cursor.execute(sql)
    ...
    for tuple_track_info in data:
        ...
        sql_update = """UPDATE temporary_boomplay_track_name_cleaned_data SET track_name_cleaned=%s,feat_artist_name=%s,version=%s,state=1 WHERE track_id=%s AND state = 0"""
