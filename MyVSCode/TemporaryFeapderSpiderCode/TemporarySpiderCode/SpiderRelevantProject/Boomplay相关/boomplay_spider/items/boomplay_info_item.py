...
class BoomplayArtistAlbumTrackCountInfoDataItem(UpdateItem):
    """
    歌手web页面下歌曲及专辑记录表
    """

    __table_name__ = "boomplay_artist_album_track_count_info_data"
    __unique_key__ = ['boomplay_artist_id','album_counts','source','batch']
    __update_key__ = ['boomplay_artist_id','album_counts','source','batch']

    def __init__(self, *args, **kwargs):
        # self.id = kwargs.get('id')
        self.boomplay_artist_id = None  # 提供的歌手艺人id
        # self.track_counts = kwargs.get('track_counts')
        self.album_counts = None
        self.source = None  # 数据来源（非洲/非洲以外）
        self.batch = None
        # self.gtime = kwargs.get('gtime')


class BoomplayTrackStatusFailedBatchDataItem(UpdateItem):
    """
    This class was generated by feapder
    command: feapder create -i boomplay_track_status_failed_batch_data 1
    """

    __table_name__ = "boomplay_track_status_failed_batch_data"
    __unique_key__ = ['track_id','batch']
    __update_key__ = ['track_id','failed_info','batch']
