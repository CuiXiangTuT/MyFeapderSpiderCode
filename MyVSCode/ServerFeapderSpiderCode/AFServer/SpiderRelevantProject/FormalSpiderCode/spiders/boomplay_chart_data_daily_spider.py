...
class BoomplayChartDataDailySpider(feapder.BatchSpider):
    ...
    def parse(self, request, response):
        ...
        """
        # 每日榜单数据item
        chart_data = ChartDataDailyBoomplayAfItem()
        # 1.抓取平台
        ...
        for per_track_info in li_list:
            ...
            chart_data_track_info['note'] = '非洲榜单'
            """
            ...
            album_info_task_item['album_id'] = per_track_info.xpath(
                './/a[@class="albumName"]/@href').extract_first().split(
                    '/')[-1].split('?')[0]
            """
            ...
            yield self.add_new_task('boomplay_artist_info_batch_task',artist_info_task_item.to_dict)
            yield album_info_task_item
            yield self.add_new_task('boomplay_album_info_batch_task',album_info_task_item.to_dict)
