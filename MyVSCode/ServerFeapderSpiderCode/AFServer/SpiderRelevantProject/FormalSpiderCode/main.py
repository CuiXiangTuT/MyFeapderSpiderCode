...
def crawl_boomplay_chart_data_daily_spider(args):
    """
    1-Boomplay榜单页数据采集爬虫（非洲采集）
    """
    spider = boomplay_chart_data_daily_spider.BoomplayChartDataDailySpider(
        task_table="chart_boomplay_batch_task",  # mysql中的任务表
        batch_record_table="chart_af_boomplay_batch_task_record",  # mysql中的批次记录表
        batch_name="Boomplay榜单页数据采集爬虫（非洲采集）",  # 批次名字
        ...
        redis_key="feapder:af_chart_boomplay_batch_task",  # redis中存放request等信息的根key
...
def crawl_boomplay_artist_info_spider(args):
    """
    2-Boomplay歌手详情页信息采集（非洲采集）
    """
    spider = boomplay_artist_info_spider.BoomplayArtistInfoSpider(
        task_table="boomplay_artist_info_batch_task",  # mysql中的任务表
        batch_record_table=
        "boomplay_af_artist_info_batch_task_record",  # mysql中的批次记录表
        batch_name="Boomplay歌手详情页信息采集（非洲采集）",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=[
        ...
        redis_key=
        "feapder:af_boomplay_artist_info_batch_task",  # redis中存放request等信息的根key
        task_state="state",  # mysql中任务状态字段
        task_condition="state = -1", # 任务条件 用于从一个大任务表中挑选出数据自己爬虫的任务，即where后的条件语句
    )
    if args == 1:
        spider.start_monitor_task()
    elif args == 2:
...
def crawl_boomplay_artist_info_task_spider(args):
    """
    3-Boomplay歌手详情页歌曲、专辑任务采集（非洲采集）
    """
    spider = boomplay_artist_info_task_spider.BoomplayArtistInfoTaskSpider(
        task_table="boomplay_artist_info_batch_task",  # mysql中的任务表
        batch_record_table=
        "boomplay_af_artist_album_track_task_record",  # mysql中的批次记录表
        batch_name="Boomplay歌手详情页歌曲、专辑任务采集（非洲采集）",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id", "boomplay_artist_id"],  # 需要获取任务表里的字段名，可添加多个
        redis_key=
        "feapder:af_boomplay_artist_album_track_task",  # redis中存放request等信息的根key
        task_state="af_artist_album_track_state",  # mysql中任务状态字段
    )
    if args == 1:
        spider.start_monitor_task()
    elif args == 2:
...
def crawl_boomplay_album_info_task_spider(args):
    """
    4-Boomplay专辑详情页信息采集（非洲采集）
    """
    spider = boomplay_album_info_spider.BoomplayAlbumInfoSpider(
        task_table="boomplay_album_info_batch_task",  # mysql中的任务表
        batch_record_table=
        "boomplay_af_album_info_batch_task_record",  # mysql中的批次记录表
        batch_name="Boomplay专辑详情页信息采集（非洲采集）",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id", "album_id"],  # 需要获取任务表里的字段名，可添加多个
        redis_key=
        "feapder:af_boomplay_album_info_batch_task",  # redis中存放request等信息的根key
        task_state="af_state",  # mysql中任务状态字段
    )
    if args == 1:
        spider.start_monitor_task()
...
def crawl_boomplay_track_info_task_spider(args):
    """
    5-Boomplay歌曲详情页信息采集（非洲采集）
    """
    spider = boomplay_track_info_spider.BoomplayTrackInfoSpider(
        task_table="boomplay_track_info_batch_task",  # mysql中的任务表
        batch_record_table=
        "boomplay_af_track_info_batch_task_record",  # mysql中的批次记录表
        batch_name="Boomplay歌曲详情页信息采集（非洲采集）",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id", "track_id"],  # 需要获取任务表里的字段名，可添加多个
        redis_key=
        "feapder:af_boomplay_track_info_batch_task",  # redis中存放request等信息的根key
        task_state="state",  # mysql中任务状态字段
        task_condition="state = -1", # 任务条件 用于从一个大任务表中挑选出数据自己爬虫的任务，即where后的条件语句
    )
    if args == 1:
...
def crawl_boomplay_track_views_task_spider(args):
    """
    6-Boomplay歌曲播放量信息采集（非洲采集）
    """
    spider = boomplay_track_views_spider.BoomplayTrackViewsSpider(
        task_table="boomplay_track_info_batch_task",  # mysql中的任务表
        batch_record_table="boomplay_af_track_views_spider_record",  # mysql中的批次记录表
        batch_name="Boomplay歌曲播放量信息采集（非洲采集）",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id", "track_id"],  # 需要获取任务表里的字段名，可添加多个
        redis_key=
        "feapder:af_boomplay_track_views_batch_task",  # redis中存放request等信息的根key
        task_state="views_state",  # mysql中任务状态字段
        task_condition="views_state = -1", # 任务条件 用于从一个大任务表中挑选出数据自己爬虫的任务，即where后的条件语句
    )
    if args == 1:
        spider.start_monitor_task()
    elif args == 2:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        nargs=1,
        help="Boomplay歌手详情页歌曲、专辑任务采集（非洲采集）",
        choices=[1, 2, 3],
