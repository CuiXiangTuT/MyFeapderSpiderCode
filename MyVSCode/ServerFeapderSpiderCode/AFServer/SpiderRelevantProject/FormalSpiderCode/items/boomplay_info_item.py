...
class ChartDataDailyBoomplayAfItem(UpdateItem):
    """
    说明：Boomplay榜单数据表【chart_data_daily_boomplay_af】
    作用：用于存放每日抓取的榜单歌曲表信息
    """
...
class BoomplayTrackNameCleanedDataItem(UpdateItem):
    ...
    command: feapder create -i boomplay_track_name_cleaned_data 1
    """

    __table_name__ = "boomplay_track_name_cleaned_data"
    __unique_key__ = ['track_id']
    __update_key__ = ['track_id','track_name']

    def __init__(self, *args, **kwargs):
        # self.id = kwargs.get('id')
        self.track_id = None
        self.track_name = None
        # self.track_name_cleaned = kwargs.get('track_name_cleaned')  # 歌曲名清理后
        # self.feat_artist_name = kwargs.get('feat_artist_name')
