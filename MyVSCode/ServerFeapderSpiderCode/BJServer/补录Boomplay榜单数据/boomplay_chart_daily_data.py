...
@description：
    ...
    逻辑如下：
        1.获取当前日期下chart_data_daily表中，chart_site='boomplay'的数据（默认是北京，每个地区榜单不足100）
        2.从music_data_hk中获取当前日期下的榜单数据，理论结果为400
        3.如果能从music_data_hk中获取到相关数据，那么将【当前日期】下的chart_data_daily关于boomplay数据清除
          如果不能，则保留当前榜单内容
"""
    ...
    if str(result_ghana)==str(result_kenya)==str(result_tanzania)==str(result_nigeria)=='100':
        ...
        if str(result_chart_data_daily)=='400':
            ...
            INSERT INTO music_data.chart_data_daily(
                ...
                FROM music_data_hk.chart_data_daily_boomplay
        ...
        if int(result_chart_data_daily)>0:
            ...
            # 当前日期下，不存在boomplay榜单数据，那么使用music_data_bj中的数据做补充
            sql_music_data_bj_chart = '''
            INSERT INTO music_data.chart_data_daily(
            table_No
            ,`rank`
            ,song_id
            ...
            ,'' AS chart_update_date
            ,'' AS chart_unique_name
            FROM music_data_bj.chart_data_daily_boomplay
            WHERE  batch = CURRENT_DATE;
        '''
        music_data_hk_cursor.execute(sql_music_data_bj_chart)
        music_data_hk_conn.commit()
        today = datetime.datetime.now().strftime("%Y-%m-%d")
