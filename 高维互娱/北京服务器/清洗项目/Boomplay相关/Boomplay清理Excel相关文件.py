...
def split_feat_func(track_name):
    ...
    feat_pattern = 'ft\.(.*)|ft\. (.*)|ft (.*)|feat\.(.*)|feat\. (.*)|feat (.*)|featuring.(.*)|featuring(.*)'
    if re.findall(feat_pattern,track_name, re.IGNORECASE):
        ...
        if '(' in feat_name and ')' in feat_name:
            left_brackets = feat_name.strip().find('(')
            right_brackets = feat_name.strip().find(')')
            if left_brackets>right_brackets:
                feat_name = feat_name[:right_brackets]
                if feat_name.strip().endswith(']') and '[' not in feat_name:
                    feat_name = feat_name.replace(']','')
                    return feat_name
                else:
                    return feat_name
...
def cleaned_track_name_func(track_name,feat_name,version,mid_version):
    ...
    if track_name_cleaned.strip().endswith("[Ft."):
        track_name_cleaned = track_name_cleaned.replace("[Ft.", "")
    pattern = r'\((.*)\)|\[(.*)\]|\{(.*)\}'
    track_name_cleaned = track_name_cleaned.replace('[','(').replace(']',')')
...
def main():
    ...
    cursor = conn.cursor()
    sql='SELECT boomplay_track_id,capture_track_name_boomplay FROM `ghnan_new_track`'
    cursor.execute(sql)
    ...
    for tuple_track_info in data:
        ...
        sql_update = """
        UPDATE `ghnan_new_track` SET capture_track_name_manu_cleaned="%s",feat_artist_name_boomplay="%s",version="%s" WHERE boomplay_track_id="%s"
        """%(track_name_cleaned.lower().strip(),feat_name.lower().strip(),version.lower().strip(),tuple_track_info[0])
