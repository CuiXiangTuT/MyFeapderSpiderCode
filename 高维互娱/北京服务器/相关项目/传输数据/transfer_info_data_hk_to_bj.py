...
def transfer_info_data_hk_to_bj():
    ...
    # 昨日批次
    past_week = [(current_date + datetime.timedelta(days=-i)).strftime("%Y-%m-%d") for i in range(1,8)]


    # 抽取歌手信息数据
    sql_artist_info_data = """
    SELECT gmg_artist_id,gmg_artist_name,crawl_artist_name,boomplay_artist_id,crawl_boomplay_artist_id,boomplay_artist_name,boomplay_artist_certification,batch,boomplay_artist_image,boomplay_artist_info,ranking_current,ranking_alltime,country_region,artist_favorite_count,artist_share_count,artist_comment_count FROM boomplay_artist_info_batch_data
    """
    hk_cursor.execute(sql_artist_info_data)
    artist_info_data = hk_cursor.fetchall()
    df_artist_info_data = pandas.DataFrame(artist_info_data,columns=['gmg_artist_id','gmg_artist_name','crawl_artist_name','boomplay_artist_id','crawl_boomplay_artist_id','boomplay_artist_name','boomplay_artist_certification','batch','boomplay_artist_image','boomplay_artist_info','ranking_current','ranking_alltime','country_region','artist_favorite_count','artist_share_count','artist_comment_count'])

    # 抽取专辑信息数据
    sql_album_info_data = "SELECT crawl_album_id,album_id,album_name,album_type,album_image,album_track_count,album_info,boomplay_artist_id,album_favorite_count,album_share_count,album_comment_count,batch FROM boomplay_album_info_batch_data WHERE batch IN ({})"
    hk_cursor.execute(sql_album_info_data,str(past_week)[1:-1])
    ...
    # 抽取歌曲信息数据
    sql_track_info_data = "SELECT crawl_track_id,track_id,track_name,track_type,track_image,album_id,duration,boomplay_artist_id,boomplay_artist_name,capture_artist_id,capture_artist_name,capture_artist_image,capture_album_id,capture_album_name,capture_album_image,track_favorite_count,track_share_count,track_comment_count,genre,publish_date,lyrics_url,lyrics,batch FROM boomplay_track_info_batch_data WHERE batch IN ({})"
    hk_cursor.execute(sql_track_info_data,str(past_week)[1:-1])
    ...
    # 抽取歌曲播放量
    sql_track_views_data = "SELECT track_id,views,batch,crawl_frequency FROM boomplay_track_views_batch_data WHERE batch IN ({})"
    hk_cursor.execute(sql_track_views_data,str(past_week)[1:-1])
    ...
    for i in range(len(df_artist_info_data)):
        ...
        ranking_alltime = df_artist_info_data.iloc[i]['ranking_alltime']
        ...
        bj_artist_info_sql = "INSERT IGNORE INTO boomplay_artist_info_batch_data(gmg_artist_id,gmg_artist_name,crawl_artist_name,boomplay_artist_id,crawl_boomplay_artist_id,boomplay_artist_name,boomplay_artist_certification,batch,boomplay_artist_image,boomplay_artist_info,ranking_current,ranking_alltime,country_region,artist_favorite_count,artist_share_count,artist_comment_count) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
        bj_af_cursor.execute(bj_artist_info_sql,[gmg_artist_id,gmg_artist_name,crawl_artist_name,boomplay_artist_id,crawl_boomplay_artist_id,boomplay_artist_name,boomplay_artist_certification,batch,boomplay_artist_image,boomplay_artist_info,ranking_current,ranking_alltime,country_region,artist_favorite_count,artist_share_count,artist_comment_count])
        bj_af_conn.commit()
    ...
    for i in range(len(df_chart_data_daily)):
        ...
        bj_chart_data_daily_sql = "INSERT IGNORE INTO chart_data_daily_boomplay(`rank`,song_id,song_name,chart_artist_id,chart_artist_name,album_id,album_name,duration,chart_region,crawl_chart_country,batch,chart_site,chart_type,update_frequency,chart_language,chart_segment,chart_release_date,ranking_state_change,chart_name,note) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
