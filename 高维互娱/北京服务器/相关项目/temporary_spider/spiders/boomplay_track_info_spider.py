...
class BoomplayTrackInfoSpider(feapder.BatchSpider):
    ...
    def start_requests(self, task):
        ...
        yield feapder.Request(url=url, task_id=task_id,
    ...
    def parse(self, request, response):
        ...
        if response.xpath('//div[@class="noData"]/div[@class="text"]'):
            ...
            # 2-歌曲id
            track_info_batch_data_item["track_id"] = json_data["@id"].split("/")[-1]
            # 3-歌曲名
            track_info_batch_data_item["track_name"] = json_data["name"].replace("&amp;","&").replace("&#039;","'").lower().strip()
            # 4-歌曲类型
            track_info_batch_data_item["track_type"] = str(json_data["@type"]).lower()
            # 5-歌曲封面
            track_info_batch_data_item["track_image"] = json_data["image"]
            # 6-专辑id
            track_info_batch_data_item["album_id"] = json_data["inAlbum"]["@id"].split("/")[-1]
            # 7-歌曲时长
            duration = json_data["duration"].replace('PT', '').replace('M', ':').replace(
                "S", '')
            h, m, s = ("00:" + duration).strip().split(":") if len(duration.split(":")) == 2 else duration.strip().split(":")
            track_info_batch_data_item["duration"] = int(h) * 3600 + int(m) * 60 + int(s)
            # 8-歌手id
            track_info_batch_data_item["boomplay_artist_id"] = json_data["byArtist"][0]["@id"].split('/')[-1]
            # 9-歌手名boomplay_artist_id
            track_info_batch_data_item["boomplay_artist_name"] = str(json_data["byArtist"][0]["name"]).replace("&amp;","&").replace("&#039;","'").lower()
            # 10-歌手名capture_artist_name
            artist_name_get = ''.join(response.xpath('.//a[@class="ownerWrap"]/strong/text()').extract()).strip().lower()
            track_info_batch_data_item["capture_artist_name"] = str(artist_name_get.split(":",1)[1]).strip().replace("&amp;","&").replace("&#039;","'").lower() if "artist" in artist_name_get else artist_name_get.lower()
            # 11-歌手id
            track_info_batch_data_item["capture_artist_id"] = response.xpath('.//div[@class="ownerWrapOutForSong"]/a/@href').extract_first().split("/")[-1]
            # 12-歌手封面
            track_info_batch_data_item["capture_artist_image"] = response.xpath(
                './/div[contains(@class,"default") and contains(@class,"default_artist ")]/div/@style').extract_first().split(
                "url")[1][2:-2]
            try:
                # 13-专辑封面
                track_info_batch_data_item["capture_album_image"] = response.xpath(
                    './/div[contains(@class,"default") and contains(@class,"default_album")]/div/@style').extract_first().split(
                    '(')[1][1:-2]
            except:
                track_info_batch_data_item["capture_album_image"] = ""
            try:
                # 14-专辑名
                album_name_get = ''.join(response.xpath(
                    './/a[contains(@class,"ownerWrap_album") and contains(@class,"ownerWrap")]/strong/text()').extract()).strip().lower()
                track_info_batch_data_item["capture_album_name"] = album_name_get.split(":",1)[1].replace("&amp;","&").replace("&#039;","'") if "album" in album_name_get else album_name_get
            except:
                track_info_batch_data_item["capture_album_name"] = ""
            try:
                # 15-专辑id
                track_info_batch_data_item["capture_album_id"] = response.xpath(
                    './/a[contains(@class,"ownerWrap_album") and contains(@class,"ownerWrap")]/@href').extract_first().split(
                    "/")[-1]
            except:
            ...
            try:
                # 19-曲风
                track_info_batch_data_item["genre"] = response.xpath('.//section[@class="songDetailInfo"]/ul/li[1]/span/text()').extract_first()
            except:
                track_info_batch_data_item["genre"] = ""
            try:
                # 20-发行年份
                track_info_batch_data_item["publish_date"] = response.xpath(
                    './/section[@class="songDetailInfo"]/ul/li[2]/span/text()').extract_first()
            except:
                track_info_batch_data_item["publish_date"] = ''
            try:
                # 21-歌词链接
                # 获取歌曲歌词
                track_info_batch_data_item["lyrics_url"] = response.xpath('.//div[@class="lyrics"]/a/@href').extract_first()
                track_info_data_url = track_info_batch_data_item["lyrics_url"]
                yield feapder.Request(url=track_info_data_url, callback=self.parse_lyrics_info, track_info_batch_data_item=track_info_batch_data_item,task_note=request.task_note)
            except:
            ...
            # 添加歌手id至歌手任务表中
            artist_info_batch_task_item['boomplay_artist_id'] = track_info_batch_data_item["capture_artist_id"]
            artist_info_batch_task_item['boomplay_artist_name'] = track_info_batch_data_item["boomplay_artist_name"]
    ...
    def parse_lyrics_info(self, request, response):
        ...
        track_info_batch_data_item["lyrics"] = ','.join(response.xpath('.//div[@class="lyrics"]/p/text()').extract())
        track_info_batch_data_item["batch"] = self.batch_date
        yield track_info_batch_data_item
