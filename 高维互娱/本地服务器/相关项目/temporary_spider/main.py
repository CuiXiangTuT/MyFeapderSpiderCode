...
def crawl_boomplay_chart_data_daily_spider(args):
    ...
    spider = boomplay_chart_data_daily_spider.BoomplayChartDataDailySpider(
        ...
        task_keys=["id", "region_en_abbreviation", "chart_region", "crawl_chart_country", "chart_site", "chart_type", "update_frequency", "chart_language", "chart_segment", "front-end web"],  # 需要获取任务表里的字段名，可添加多个
        redis_key="feapder:chart_boomplay_batch_task",  # redis中存放request等信息的根key
...
def crawl_boomplay_artist_info_spider(args):
    ...
    spider = boomplay_artist_info_spider.BoomplayArtistInfoSpider(
        ...
        batch_record_table="boomplay_artist_info_batch_task_record",  # mysql中的批次记录表
        ...
        redis_key="feapder:boomplay_artist_info_batch_task",  # redis中存放request等信息的根key
        task_state="state",  # mysql中任务状态字段
    )
    if args == 1:
        spider.start_monitor_task()
...
def crawl_boomplay_artist_info_task_spider(args):
    ...
    spider = boomplay_artist_info_task_spider.BoomplayArtistInfoTaskSpider(
        task_table="boomplay_artist_info_batch_task",  # mysql中的任务表
        batch_record_table="boomplay_artist_album_track_task_record",  # mysql中的批次记录表
        batch_name="Boomplay歌手详情页歌曲、专辑任务采集",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id", "boomplay_artist_id"],  # 需要获取任务表里的字段名，可添加多个
        redis_key="feapder:boomplay_artist_album_track_task",  # redis中存放request等信息的根key
        task_state="artist_album_track_state",  # mysql中任务状态字段
    )
    if args == 1:
        spider.start_monitor_task()
    elif args == 2:
...
def crawl_boomplay_album_info_task_spider(args):
    ...
    spider = boomplay_album_info_spider.BoomplayAlbumInfoSpider(
        ...
        batch_record_table="boomplay_album_info_batch_task_record",  # mysql中的批次记录表
        batch_name="Boomplay专辑详情页信息采集",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id", "album_id"],  # 需要获取任务表里的字段名，可添加多个
        redis_key="feapder:boomplay_album_info_batch_task",  # redis中存放request等信息的根key
        task_state="state",  # mysql中任务状态字段
    )
    if args == 1:
        spider.start_monitor_task()
...
def crawl_boomplay_track_info_task_spider(args):
    ...
    spider = boomplay_track_info_spider.BoomplayTrackInfoSpider(
        ...
        batch_record_table="boomplay_track_info_batch_task_record",  # mysql中的批次记录表
        batch_name="Boomplay歌曲详情页信息采集",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id", "track_id"],  # 需要获取任务表里的字段名，可添加多个
        redis_key="feapder:boomplay_track_info_batch_task",  # redis中存放request等信息的根key
        task_state="state",  # mysql中任务状态字段
    )
    if args == 1:
        spider.start_monitor_task()
...
def crawl_boomplay_track_views_task_spider(args):
    ...
    spider = boomplay_track_views_spider.BoomplayTrackViewsSpider(
        ...
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id", "track_id"],  # 需要获取任务表里的字段名，可添加多个
        redis_key="feapder:boomplay_track_views_batch_task",  # redis中存放request等信息的根key
...
def crawl_tiktok_info_selenium_spider(args):
    """
    7-使用selenium抓取tiktok的user_name
    """
    spider = tiktok_info_selenium_spider.TiktokInfoSeleniumSpider(
        task_table="tiktok_user_name_batch_task",  # mysql中的任务表
        batch_record_table="tiktok_user_name_batch_task_record",  # mysql中的批次记录表
        batch_name="Tik Tok：使用selenium抓取tiktok的user_name",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id", "gmg_artist_id","gmg_artist_name","user_id",'sec_uid',"tiktok_url"],  # 需要获取任务表里的字段名，可添加多个
        redis_key="feapder:tiktok_user_name_batch_task",  # redis中存放request等信息的根key
        task_state="state",  # mysql中任务状态字段
    )
    if args == 1:
        spider.start_monitor_task()
...
if __name__ == "__main__":
    ...
    # 7-使用selenium抓取tiktok的user_name
    parser.add_argument(
        "--crawl_tiktok_info_selenium_spider",
        type=int,
        nargs=1,
        help="使用selenium抓取tiktok的user_name",
        choices=[1, 2, 3],
        function=crawl_tiktok_info_selenium_spider,
    )
