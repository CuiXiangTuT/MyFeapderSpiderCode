...
def transfer_info_data_hk_to_bj():
    ...
    # 抽取专辑信息数据
    sql_album_info_data = "SELECT crawl_album_id,album_id,album_name,album_type,album_image,album_track_count,album_info,boomplay_artist_id,album_favorite_count,album_share_count,album_comment_count,batch FROM boomplay_album_info_batch_data WHERE batch = %s"
    hk_cursor.execute(sql_album_info_data,[prev_batch])
    ...
    # 抽取歌曲播放量
    sql_track_views_data = "SELECT track_id,views,batch,crawl_frequency FROM boomplay_track_views_batch_data WHERE batch = %s"
    hk_cursor.execute(sql_track_views_data,[prev_batch])
