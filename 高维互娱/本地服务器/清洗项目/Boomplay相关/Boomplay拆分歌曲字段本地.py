...
def main():
    ...
    # SELECT track_id,track_name FROM `boomplay_track_info_batch_data`
    # """
    # cursor.execute(insert_sql)
    sql='SELECT boomplay_track_id,capture_track_name_boomplay FROM `Sheet2` WHERE boomplay_track_id IS NOT NULL'
    cursor.execute(sql)
    ...
    for tuple_track_info in data:
        ...
        print("-------------------------------------------------------------")
        sql_insert = """
        INSERT IGNORE INTO boomplay_track_name_cleaned_data(`track_id`,`track_name`,`track_name_cleaned`,`feat_artist_name`,`version`)values ("{}","{}","{}","{}","{}")""".format(tuple_track_info[0].strip(),tuple_track_info[1].strip(),track_name_cleaned.strip(),feat_name,version.strip())
        # sql_update = """
        # UPDATE boomplay_track_name_cleaned_data SET track_name_cleaned="{}",feat_artist_name="{}",version="{}" WHERE track_id="{} AND state = 0"
        # """.format(track_name_cleaned,feat_name,version,tuple_track_info[0])
