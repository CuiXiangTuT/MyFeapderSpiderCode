...
class CrawlBoomplayArtistInfoSpider(feapder.BatchSpider):
    ...
    def add_task(self):
        ...
        """
        update_sql = """
        UPDATE `b_gmg_boomplay_artist_info_batch_task` b
        ...
        """
        insert_sql = """
        INSERT IGNORE INTO `b_gmg_boomplay_artist_info_batch_task`
        ...
        WHERE site='boomplay' AND `id` NOT IN (
            SELECT boomplay_artist_id FROM `b_gmg_boomplay_artist_info_batch_task`
        )
        """
    ...
    def parse(self, request, response):
        ...
        if str(request.task_boomplay_artist_id) == "43630":
            boompplay_artist_info_crawl_situation_record_batch_data_item = GmgBoomplayArtistInfoCrawlSituationRecordBatchDataItem()
            ...
            if is_exist_info == 'artistsDetails':
                # ========================================================================
                # 一、歌手信息相关字段
                boomplay_artist_info_batch_data_item = GmgBoomplayArtistInfoBatchDataItem()
                # 1.gmg_artist_id
                ...
                """.format(
                    task_table="b_gmg_boomplay_artist_info_batch_task",
