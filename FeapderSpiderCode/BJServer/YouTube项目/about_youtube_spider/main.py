...
def crawl_search_youtube_info_spider(args):
    """
    BatchSpider爬虫
    """
    spider = search_youtube_info_spider.SearchYoutubeInfoSpider(
        task_table="search_youtube_info_task",  # mysql中的任务表
        batch_record_table="search_youtube_info_task_record",  # mysql中的批次记录表
        batch_name="搜索youtube信息及播放量",  # 批次名字
        batch_interval=1,  # 批次时间 天为单位 若为小时 可写 1 / 24
        task_keys=["id", "artist_id","artist_name",'track_id','track_name'],  # 需要获取任务表里的字段名，可添加多个
        redis_key="feapder:search_youtube_info_task",  # redis中存放request等信息的根key
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--crawl_search_youtube_info_spider",
        type=int,
        nargs=1,
        help="搜索youtube信息及播放量",
        choices=[1, 2, 3],
        function=crawl_search_youtube_info_spider,
    )
